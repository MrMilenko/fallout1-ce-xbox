...
{
    // DbgPrint("> main_menu_create: start\n");
    ...
    if (main_menu_created) {
        // DbgPrint("> main_menu_create: already created, returning\n");
        return 0;
    }

    // DbgPrint("> main_menu_create: loading color.pal\n");
    ...
    // DbgPrint("> main_menu_create: calling win_add (%d, %d)\n", mainMenuWindowX, mainMenuWindowY);
    ...
    if (main_window == -1) {
        // DbgPrint("!! main_menu_create: win_add failed\n");
        return main_menu_fatal_error();
    }

    // DbgPrint("> main_menu_create: retrieving window buffer\n");
    main_window_buf = win_get_buf(main_window);

    // DbgPrint("> main_menu_create: loading background art (FID 140)\n");
    ...
    if (background_data == NULL) {
        // DbgPrint("!! main_menu_create: background art lock failed\n");
        return main_menu_fatal_error();
    }

    // DbgPrint("> main_menu_create: copying background to buffer\n");
    ...
    // DbgPrint("> main_menu_create: setting font to 100\n");
    ...
    if (message_search(&misc_message_file, &msg)) {
        // DbgPrint("> main_menu_create: printing copyright\n");
        win_print(main_window, msg.text, 0, 15, 460, colorTable[21204] | 0x4000000 | 0x2000000);
    } else {
        // DbgPrint("!! main_menu_create: message 14 not found\n");
    }

    // Version string.
    // DbgPrint("> main_menu_create: getting version string\n");
    char version[VERSION_MAX];
    getverstr(version, sizeof(version));
    len = text_width(version);
    // DbgPrint("> main_menu_create: printing version (len = %d)\n", len);
    win_print(main_window, version, 0, 615 - len, 460, colorTable[21204] | 0x4000000 | 0x2000000);

    // Load button graphics.
    // DbgPrint("> main_menu_create: loading button_up_data (FID 299)\n");
    fid = art_id(OBJ_TYPE_INTERFACE, 299, 0, 0, 0);
    button_up_data = art_ptr_lock_data(fid, 0, 0, &button_up_key);
    if (button_up_data == NULL) {
        // DbgPrint("!! main_menu_create: button_up_data lock failed\n");
        return main_menu_fatal_error();
    }

    // DbgPrint("> main_menu_create: loading button_down_data (FID 300)\n");
    ...
    if (button_down_data == NULL) {
        // DbgPrint("!! main_menu_create: button_down_data lock failed\n");
        return main_menu_fatal_error();
    }

    // DbgPrint("> main_menu_create: registering buttons\n");
    ...
    for (int index = 0; index < MAIN_MENU_BUTTON_COUNT; index++) {
        ...
        if (buttons[index] == -1) {
            // DbgPrint("!! main_menu_create: win_register_button failed at index %d\n", index);
            return main_menu_fatal_error();
        }
    ...
    // DbgPrint("> main_menu_create: setting font to 104\n");
    text_font(104);

    // DbgPrint("> main_menu_create: rendering button labels\n");
    for (int index = 0; index < MAIN_MENU_BUTTON_COUNT; index++) {
        msg.num = 9 + index;
        if (message_search(&misc_message_file, &msg)) {
            len = text_width(msg.text);
            // DbgPrint(">> msg #%d = '%s' (len=%d)\n", msg.num, msg.text, len);
            ...
            // DbgPrint("!! main_menu_create: message %d not found\n", msg.num);
        }
    }

    // DbgPrint("> main_menu_create: restoring old font\n");
    text_font(oldFont);

    main_menu_created = true;
    main_menu_is_hidden = true;

    // DbgPrint("> main_menu_create: done\n");
    return 0;
}
...
{
    // DbgPrint("main_menu_hide: called (animate=%d)\n", animate);

    if (!main_menu_created) {
        // DbgPrint("main_menu_hide: main_menu_created == false, skipping\n");
        return;
    }

    if (main_menu_is_hidden) {
        // DbgPrint("main_menu_hide: main_menu_is_hidden == true, skipping\n");
        return;
    }

    // DbgPrint("main_menu_hide: calling soundUpdate()\n");
    soundUpdate();

    if (animate) {
        // DbgPrint("main_menu_hide: calling palette_fade_to(black_palette)\n");
        palette_fade_to(black_palette);
        // DbgPrint("main_menu_hide: calling soundUpdate() after fade\n");
        soundUpdate();
    }

    // DbgPrint("main_menu_hide: calling win_hide(main_window=%d)\n", main_window);
    win_hide(main_window);
    // DbgPrint("main_menu_hide: win_hide completed\n");

    main_menu_is_hidden = true;
    // DbgPrint("main_menu_hide: complete\n");
}
