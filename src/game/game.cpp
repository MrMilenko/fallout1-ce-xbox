...
int game_init(const char* windowTitle, bool isMapper, int font, int flags, int argc, char** argv)
{
    ...
    DbgPrint("game_init: entered, mounted E\n");

    // Show XBE path info
    char xbeNtPath[260];
    nxGetCurrentXbeNtPath(xbeNtPath);
    DbgPrint("DEBUG: Loaded XBE NT path: %s\n", xbeNtPath);

    if (strncmp(xbeNtPath, "\\Device\\Harddisk0\\Partition1", 28) == 0) {
        char xbeDosPath[260];
        snprintf(xbeDosPath, sizeof(xbeDosPath), "E:%s", xbeNtPath + 28);
        DbgPrint("DEBUG: Simulated DOS path: %s\n", xbeDosPath);
    }

    char path[COMPAT_MAX_PATH];

    if (gmemory_init() == -1) {
        DbgPrint("game_init: gmemory_init failed\n");
        return -1;
    }
    DbgPrint("game_init: gmemory_init succeeded\n");

    DbgPrint("game_init: calling gconfig_init\n");
    gconfig_init(isMapper, argc, argv);
    DbgPrint("game_init: gconfig_init returned\n");

    game_in_mapper = isMapper;

    DbgPrint("game_init: calling game_init_databases\n");
    if (game_init_databases() == -1) {
        DbgPrint("game_init: game_init_databases failed\n");
        gconfig_exit(false);
        return -1;
    }

    DbgPrint("game_init: calling win_set_minimized_title\n");
    win_set_minimized_title(windowTitle);

    VideoOptions video_options = {640, 480, true, 1};

    DbgPrint("game_init: loading resolution config\n");
    Config resolutionConfig;
    if (config_init(&resolutionConfig)) {
        if (config_load(&resolutionConfig, "E:\\Games\\Fallout\\f1_res.ini", false)) {
            int screenWidth;
    ...
    DbgPrint("game_init: calling initWindow\n");
    initWindow(&video_options, flags);
    DbgPrint("game_init: calling palette_init\n");
    palette_init();

    if (!game_in_mapper) {
        DbgPrint("game_init: showing splash screen\n");
        game_splash_screen();
    }
    ...
    if (gsound_init() != 0) DbgPrint("game_init: Sound initialization failed\n");
    DbgPrint(">gsound_init");

    initMovie();        DbgPrint(">initMovie");
    if (gmovie_init())  { DbgPrint("Failed on gmovie_init\n"); return -1; }
    DbgPrint(">gmovie_init");

    if (moviefx_init()) { DbgPrint("Failed on moviefx_init\n"); return -1; }
    DbgPrint(">moviefx_init");

    if (iso_init())     { DbgPrint("Failed on iso_init\n"); return -1; }
    DbgPrint(">iso_init");

    if (gmouse_init())  { DbgPrint("Failed on gmouse_init\n"); return -1; }
    DbgPrint(">gmouse_init");

    if (proto_init())   { DbgPrint("Failed on proto_init\n"); return -1; }
    DbgPrint(">proto_init");

    anim_init();        DbgPrint(">anim_init");

    if (scr_init())     { DbgPrint("Failed on scr_init\n"); return -1; }
    DbgPrint(">scr_init");

    if (game_load_info()) { DbgPrint("Failed on game_load_info\n"); return -1; }
    DbgPrint(">game_load_info");

    if (scr_game_init())  { DbgPrint("Failed on scr_game_init\n"); return -1; }
    DbgPrint(">scr_game_init");

    if (init_world_map()) { DbgPrint("Failed on init_world_map\n"); return -1; }
    DbgPrint(">init_world_map");

    CharEditInit();     DbgPrint(">CharEditInit");
    pip_init();         DbgPrint(">pip_init");
    InitLoadSave();     DbgPrint(">InitLoadSave");

    if (gdialog_init()) { DbgPrint("Failed on gdialog_init\n"); return -1; }
    DbgPrint(">gdialog_init");

    if (combat_init())  { DbgPrint("Failed on combat_init\n"); return -1; }
    DbgPrint(">combat_init");

    if (automap_init()) { DbgPrint("Failed on automap_init\n"); return -1; }
    DbgPrint(">automap_init");

    if (!message_init(&misc_message_file)) {
        DbgPrint("Failed on message_init\n");
        return -1;
    }
    DbgPrint(">message_init");

    snprintf(path, sizeof(path), "%s%s", msg_path, "MISC.MSG");
    DbgPrint("DEBUG: trying to load message file: %s\n", path);

    if (!message_load(&misc_message_file, path)) {
        DbgPrint("Failed on message_load\n");
        return -1;
    }
    DbgPrint(">message_load");

    if (scr_disable()) {
        DbgPrint("Failed on scr_disable\n");
        return -1;
    }
    DbgPrint(">scr_disable");

    if (init_options_menu()) {
        DbgPrint("Failed on init_options_menu\n");
        return -1;
    }
    DbgPrint(">init_options_menu");
...
int game_load_info()
{
    int rc = game_load_info_vars("DATA\\vault13.gam", "GAME_GLOBAL_VARS:", &num_game_global_vars, &game_global_vars);
    if (rc == -1) {
        DbgPrint("WARNING: Missing vault13.gam — initializing empty global vars.\n");
...
static int game_init_databases()
{
    ...
    DbgPrint("game_init_databases: entered\n");

    hashing = 0;
    main_file_name = NULL;
    patch_file_name = NULL;

    DbgPrint("game_init_databases: checking HASHING_KEY\n");
    if (config_get_value(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_HASHING_KEY, &hashing)) {
        DbgPrint("game_init_databases: HASHING_KEY enabled (%d), calling db_enable_hash_table()\n", hashing);
        db_enable_hash_table();
    } else {
        DbgPrint("game_init_databases: HASHING_KEY not found in config\n");
    }

    DbgPrint("game_init_databases: getting MASTER_DAT_KEY\n");
    config_get_string(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_MASTER_DAT_KEY, &main_file_name);
    DbgPrint("game_init_databases: MASTER_DAT_KEY = %s\n", main_file_name ? main_file_name : "NULL");
    if (main_file_name && *main_file_name == '\0') {
        DbgPrint("game_init_databases: MASTER_DAT_KEY is empty string, setting to NULL\n");
        main_file_name = NULL;
    }

    DbgPrint("game_init_databases: getting MASTER_PATCHES_KEY\n");
    config_get_string(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_MASTER_PATCHES_KEY, &patch_file_name);
    DbgPrint("game_init_databases: MASTER_PATCHES_KEY = %s\n", patch_file_name ? patch_file_name : "NULL");
    if (patch_file_name && *patch_file_name == '\0') {
        DbgPrint("game_init_databases: MASTER_PATCHES_KEY is empty string, setting to NULL\n");
        patch_file_name = NULL;
    }

    DbgPrint("game_init_databases: calling db_init for MASTER (main=%s, patch=%s)\n",
             main_file_name ? main_file_name : "NULL",
             patch_file_name ? patch_file_name : "NULL");
    ...
    if (master_db_handle == INVALID_DATABASE_HANDLE) {
        DbgPrint("game_init_databases: db_init for MASTER failed\n");
    ...
    DbgPrint("game_init_databases: MASTER db_init succeeded (handle=%d)\n", master_db_handle);

    DbgPrint("game_init_databases: getting CRITTER_DAT_KEY\n");
    config_get_string(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_CRITTER_DAT_KEY, &main_file_name);
    DbgPrint("game_init_databases: CRITTER_DAT_KEY = %s\n", main_file_name ? main_file_name : "NULL");
    if (main_file_name && *main_file_name == '\0') {
        DbgPrint("game_init_databases: CRITTER_DAT_KEY is empty string, setting to NULL\n");
        main_file_name = NULL;
    }

    DbgPrint("game_init_databases: getting CRITTER_PATCHES_KEY\n");
    config_get_string(&game_config, GAME_CONFIG_SYSTEM_KEY, GAME_CONFIG_CRITTER_PATCHES_KEY, &patch_file_name);
    DbgPrint("game_init_databases: CRITTER_PATCHES_KEY = %s\n", patch_file_name ? patch_file_name : "NULL");
    if (patch_file_name && *patch_file_name == '\0') {
        DbgPrint("game_init_databases: CRITTER_PATCHES_KEY is empty string, setting to NULL\n");
        patch_file_name = NULL;
    }

    DbgPrint("game_init_databases: calling db_init for CRITTER (main=%s, patch=%s)\n",
             main_file_name ? main_file_name : "NULL",
             patch_file_name ? patch_file_name : "NULL");
    ...
    if (critter_db_handle == INVALID_DATABASE_HANDLE) {
        DbgPrint("game_init_databases: db_init for CRITTER failed\n");
    ...
    DbgPrint("game_init_databases: CRITTER db_init succeeded (handle=%d)\n", critter_db_handle);

    DbgPrint("game_init_databases: selecting MASTER handle (%d)\n", master_db_handle);
    db_select(master_db_handle);

    DbgPrint("game_init_databases: completed successfully\n");
    return 0;
}
...
static void game_splash_screen()
{
    ...
    for (int index = 0; index < SPLASH_COUNT; index++) {
        ...
        DbgPrint("game_splash_screen(): trying %s\n", filePath);
    ...
    if (stream == NULL) {
        DbgPrint("game_splash_screen(): no splash screen found, skipping.\n");
        return;
    }

    DbgPrint("game_splash_screen(): splash file opened successfully.\n");

    // You can stub or no-op the rest here safely
    db_fclose(stream);
    return;
}
