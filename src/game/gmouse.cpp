...
int gmouse_init()
{
    // DbgPrint("gmouse_init: entered\n");

    if (gmouse_initialized) {
        // DbgPrint("gmouse_init: already initialized, returning -1\n");
        return -1;
    }

    // DbgPrint("gmouse_init: calling gmouse_3d_init()\n");
    if (gmouse_3d_init() != 0) {
        return -1;
    }
    ...
    gmouse_enabled = 1;
    // DbgPrint("gmouse_init: gmouse initialized and enabled\n");

    // DbgPrint("gmouse_init: setting cursor to MOUSE_CURSOR_ARROW (%d)\n", MOUSE_CURSOR_ARROW);
    gmouse_set_cursor(MOUSE_CURSOR_ARROW);

    // DbgPrint("gmouse_init: completed successfully, returning 0\n");
    return 0;
}
