...
{
    // DbgPrint("art_ptr_lock_data: fid=%d, frame=%d, dir=%d\n", fid, frame, direction);

    Art* art = NULL;
    if (handlePtr) {
        // DbgPrint("art_ptr_lock_data: calling cache_lock...\n");
        cache_lock(&art_cache, fid, (void**)&art, handlePtr);
        // DbgPrint("art_ptr_lock_data: returned from cache_lock, art=%p\n", art);
    }

    if (art != NULL) {
        ArtFrame* frm = frame_ptr(art, frame, direction);
        // DbgPrint("art_ptr_lock_data: got frame_ptr: %p\n", frm);

        if (frm != NULL) {
            return (unsigned char*)frm + sizeof(*frm);
        } else {
            // DbgPrint("art_ptr_lock_data: frame_ptr returned NULL\n");
        }
    } else {
        // DbgPrint("art_ptr_lock_data: art is NULL after cache_lock\n");
    }
