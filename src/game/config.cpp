...
bool config_load(Config* config, const char* filePath, bool isDb)
{
    if (config == NULL || filePath == NULL) {
        // DbgPrint("config_load: NULL config or filePath!\n");
        return false;
    }

    // DbgPrint("config_load: Attempting to open '%s' with isDb=%d\n", filePath, isDb);
    ...
    if (isDb) {
        ...
        if (stream != NULL) {
            // DbgPrint("config_load: db_fopen succeeded for '%s'\n", filePath);
            result = true;
            ...
            // DbgPrint("config_load: db_fopen FAILED for '%s'\n", filePath);
        }
    } else {
        FILE* stream = compat_fopen(filePath, "rb");
        if (stream != NULL) {
            // DbgPrint("config_load: compat_fopen succeeded for '%s'\n", filePath);
            result = true;
            ...
            // DbgPrint("config_load: compat_fopen FAILED for '%s'\n", filePath);
        }
    }
...
bool config_get_double(Config* config, const char* sectionKey, const char* key, double* valuePtr)
{
    ...
    *valuePtr = atof(stringValue);  // crude fallback
