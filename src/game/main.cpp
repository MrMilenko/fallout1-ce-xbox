...
{
    // DbgPrint("gnw_main: entered\n");

    // DbgPrint("gnw_main: calling autorun_mutex_create()\n");
    if (!autorun_mutex_create()) {
        // DbgPrint("gnw_main: autorun_mutex_create FAILED\n");
        return 1;
    }
    // DbgPrint("gnw_main: autorun_mutex_create succeeded\n");

    // DbgPrint("gnw_main: calling main_init_system()\n");
    if (!main_init_system(argc, argv)) {
        // DbgPrint("gnw_main: main_init_system FAILED\n");
        return 1;
    }
    // DbgPrint("gnw_main: main_init_system succeeded\n");

    // DbgPrint("gnw_main: playing MOVIE_IPLOGO\n");
    //gmovie_play(MOVIE_IPLOGO, GAME_MOVIE_FADE_IN);

    // DbgPrint("gnw_main: playing MOVIE_INTRO\n");
    //gmovie_play(MOVIE_INTRO, 0);

    // DbgPrint("gnw_main: calling main_menu_create()\n");
    if (main_menu_create() == 0) {
        // DbgPrint("gnw_main: main_menu_create succeeded\n");

        int language_filter = 1;
        bool done = false;

        // DbgPrint("gnw_main: calling config_get_value() for language_filter\n");
        config_get_value(&game_config, GAME_CONFIG_PREFERENCES_KEY, GAME_CONFIG_LANGUAGE_FILTER_KEY, &language_filter);

        while (!done) {
            // DbgPrint("gnw_main: main menu loop start\n");

            kb_clear();
            // DbgPrint("gnw_main: playing background music\n");
            gsound_background_play_level_music("07desert", 11);

            // DbgPrint("gnw_main: showing main menu\n");
            main_menu_show(1);

            mouse_show();
            // DbgPrint("gnw_main: calling main_menu_loop()\n");
            int mainMenuRc = main_menu_loop();
            mouse_hide();

            // DbgPrint("gnw_main: main_menu_loop returned %d\n", mainMenuRc);

            switch (mainMenuRc) {
                case MAIN_MENU_INTRO:
                    // DbgPrint("gnw_main: intro selected\n");
                    main_menu_hide(true);
                    gmovie_play(MOVIE_INTRO, GAME_MOVIE_PAUSE_MUSIC);
                    break;

                case MAIN_MENU_NEW_GAME:
                    // DbgPrint("gnw_main: new game selected\n");
                    // DbgPrint("gnw_main: Calling main_menu_hide(true)\n");
                    main_menu_hide(true);
                    // DbgPrint("gnw_main: Calling main_menu_destroy()\n");
                    main_menu_destroy();

                    if (select_character() == 2) {
                        // DbgPrint("gnw_main: character selected\n");
                ...
                case MAIN_MENU_LOAD_GAME:
                    // DbgPrint("gnw_main: load game selected\n");
                    {
                        ...
                        if (loadGameRc == -1) {
                            // DbgPrint("gnw_main: LoadGame returned -1\n");
                ...
                case MAIN_MENU_TIMEOUT:
                    // DbgPrint("gnw_main: main menu timeout\n");
                    // FALLTHROUGH
                ...
                case MAIN_MENU_CREDITS:
                    // DbgPrint("gnw_main: credits selected\n");
                    main_menu_hide(true);
                ...
                case MAIN_MENU_QUOTES:
                    if (language_filter == 0) {
                        // DbgPrint("gnw_main: quotes selected\n");
                        main_menu_hide(true);
                ...
                case MAIN_MENU_EXIT:
                case -1:
                    // DbgPrint("gnw_main: exit selected\n");
                    done = true;
                    main_menu_hide(true);
                    main_menu_destroy();
                    gsound_background_stop();
                ...
                case MAIN_MENU_SELFRUN:
                    // DbgPrint("gnw_main: selfrun selected\n");
                    main_selfrun_record();
                    break;

                default:
                    // DbgPrint("gnw_main: unknown menu return code %d\n", mainMenuRc);
                    break;
            }

            // DbgPrint("gnw_main: end of menu loop iteration\n");
        }
    } else {
        // DbgPrint("gnw_main: main_menu_create FAILED\n");
    }

    // DbgPrint("gnw_main: calling main_exit_system()\n");
    main_exit_system();

    // DbgPrint("gnw_main: calling autorun_mutex_destroy()\n");
    autorun_mutex_destroy();

    // DbgPrint("gnw_main: exiting\n");
    return 0;
}
...
static bool main_init_system(int argc, char** argv)
{
    static char fallbackArgv0[] = "E:\\Games\\Fallout\\default.xbe";

    if (argc == 0 || argv == nullptr || argv[0] == nullptr) {
        // DbgPrint("main_init_system: injecting fallback argv[0] = %s\n", fallbackArgv0);

        static char* fakeArgv[] = { fallbackArgv0 };
        argv = fakeArgv;
        argc = 1;
    } else {
        // DbgPrint("main_init_system: using provided argv[0] = %s\n", argv[0]);
    }
