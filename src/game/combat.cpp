...
int combat_init()
{
    ...
    if (obj_dude != NULL) {
        ...
        // DbgPrint("combat_init(): obj_dude is NULL — skipping AP assignment.\n");
    }
    ...
    if (!message_init(&combat_message_file)) {
        // DbgPrint("combat_init(): message_init failed.\n");
        return -1;
    }
    ...
    if (fp) {
        ...
        if (!message_load(&combat_message_file, path)) {
            // DbgPrint("combat_init(): message_load failed.\n");
            return -1;
        }
    } else {
        // DbgPrint("combat_init(): skipping message_load for %s (file missing)\n", path);
    }
