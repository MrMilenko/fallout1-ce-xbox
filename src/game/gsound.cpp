...
int gsound_init()
{
    // DbgPrint("gsound_init: entered\n");

    if (gsound_initialized) {
        // DbgPrint("gsound_init: already initialized, skipping init\n");
        return -1;
    }

    bool initialize = false;
    configGetBool(&game_config, GAME_CONFIG_SOUND_KEY, GAME_CONFIG_INITIALIZE_KEY, &initialize);
    // DbgPrint("gsound_init: GAME_CONFIG_INITIALIZE = %d\n", initialize);

    if (!initialize) {
        // DbgPrint("gsound_init: sound not requested in config, skipping init\n");
        return 0;
    }

    configGetBool(&game_config, GAME_CONFIG_SOUND_KEY, GAME_CONFIG_DEBUG_KEY, &gsound_debug);
    if (gsound_debug) {
        // DbgPrint("gsound_init: debugging enabled\n");
        // DbgPrint("gsound_init: initializing sound system...\n");
    }

    if (gsound_get_music_path(&sound_music_path1, GAME_CONFIG_MUSIC_PATH1_KEY) != 0) {
        // DbgPrint("gsound_init: failed to get music path1\n");
        return -1;
    }
    // DbgPrint("gsound_init: music path1 = %s\n", sound_music_path1);

    if (gsound_get_music_path(&sound_music_path2, GAME_CONFIG_MUSIC_PATH2_KEY) != 0) {
        // DbgPrint("gsound_init: failed to get music path2\n");
        return -1;
    }
    // DbgPrint("gsound_init: music path2 = %s\n", sound_music_path2);

    if (strlen(sound_music_path1) > 247 || strlen(sound_music_path2) > 247) {
        // DbgPrint("gsound_init: music paths too long\n");
        return -1;
    }

    if (gsound_setup_paths() != 0) {
        // DbgPrint("gsound_init: failed to setup paths\n");
        return -1;
    }

    // DbgPrint("gsound_init: registering memory allocators\n");
    soundRegisterAlloc(mem_malloc, mem_realloc, mem_free);

    // DbgPrint("gsound_init: calling soundInit()\n");
    if (soundInit(detectDevices, 24, 0x8000, 0x8000, 22050) != 0) {
        // DbgPrint("gsound_init: soundInit() failed!\n");
        return -1;
    }

    // DbgPrint("gsound_init: soundInit() succeeded\n");

    initAudiof(gsound_compressed_query);
    initAudio(gsound_compressed_query);
    // DbgPrint("gsound_init: audio init complete\n");

    int cacheSize = 0;
    config_get_value(&game_config, GAME_CONFIG_SOUND_KEY, GAME_CONFIG_CACHE_SIZE_KEY, &cacheSize);
    // DbgPrint("gsound_init: cacheSize = %d\n", cacheSize);

    if (cacheSize >= 0x40000) {
        // DbgPrint("gsound_init: config cache_size too large, must be reset in INI\n");
        return -1;
    }

    if (sfxc_init(cacheSize << 10, sound_sfx_path) != 0) {
        // DbgPrint("gsound_init: sfxc_init() failed\n");
    } else {
        // DbgPrint("gsound_init: sfxc_init() succeeded\n");
    }

    if (soundSetDefaultFileIO(gsound_open, gsound_close, gsound_read, gsound_write, gsound_seek, gsound_tell, gsound_filesize) != 0) {
        // DbgPrint("gsound_init: soundSetDefaultFileIO() failed\n");
        return -1;
    }
    // DbgPrint("gsound_init: I/O hooks set\n");

    add_bk_process(gsound_bkg_proc);
    gsound_initialized = true;
    // DbgPrint("gsound_init: added background process, init flag set\n");

    // SOUND
    bool sounds = false;
    configGetBool(&game_config, GAME_CONFIG_SOUND_KEY, GAME_CONFIG_SOUNDS_KEY, &sounds);
    // DbgPrint("gsound_init: sounds enabled? %d\n", sounds);
    if (sounds) {
        gsound_sfx_enable();
    }
    ...
    // DbgPrint("gsound_init: music enabled? %d\n", music);
    if (music) {
    ...
    // DbgPrint("gsound_init: speech enabled? %d\n", speech);
    if (speech) {
    ...
    // DbgPrint("gsound_init: master_volume = %d\n", master_volume);
    ...
    // DbgPrint("gsound_init: music_volume = %d\n", background_volume);
    ...
    // DbgPrint("gsound_init: sfx_volume = %d\n", sndfx_volume);
    ...
    // DbgPrint("gsound_init: speech_volume = %d\n", speech_volume);

    gsound_background_fade_set(0);
    background_fname_requested[0] = '\0';

    // DbgPrint("gsound_init: complete, returning 0\n");
    return 0;
}
