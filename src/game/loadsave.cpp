...
static int SaveSlot()
{
    ...
    DbgPrint("SaveSlot: Begin\n");

    gmouse_set_cursor(MOUSE_CURSOR_WAIT_PLANET);
    gsound_background_pause();

    if (!patches || strlen(patches) == 0) {
        DbgPrint("SaveSlot: Error - 'patches' is null or empty\n");
        gsound_background_unpause();
        return -1;
    }

    // Create SAVEGAME directory
    snprintf(gmpath, sizeof(gmpath), "%s\\SAVEGAME", patches);
    compat_mkdir(gmpath);

    // Create SLOT directory
    snprintf(gmpath, sizeof(gmpath), "%s\\SAVEGAME\\SLOT%.2d", patches, slot_cursor + 1);
    compat_mkdir(gmpath);

    if (SaveBackup() == -1) {
        DbgPrint("SaveSlot: Warning - SaveBackup() failed\n");
    }

    // Final path to save file
    snprintf(gmpath, sizeof(gmpath), "SAVEGAME\\SLOT%.2d\\SAVE.DAT", slot_cursor + 1);
    DbgPrint("SaveSlot: Save path: %s\n", gmpath);

    flptr = db_fopen(gmpath, "wb");
    if (flptr == NULL) {
        DbgPrint("SaveSlot: Error - Could not open save file for writing\n");

        RestoreSave();

        snprintf(gmpath, sizeof(gmpath), "SAVEGAME\\SLOT%.2d\\", slot_cursor + 1);
    ...
    if (SaveHeader(slot_cursor) == -1) {
        DbgPrint("SaveSlot: Error - SaveHeader failed\n");
        DbgPrint("SaveSlot: Header size written: %ld bytes\n", db_ftell(flptr) - pos);
    ...
    for (int i = 0; i < LOAD_SAVE_HANDLER_COUNT; i++) {
        SaveGameHandler* handler = master_save_list[i];
        if (!handler) {
            DbgPrint("SaveSlot: Warning - handler[%d] is NULL, skipping\n", i);
            continue;
        }

        pos = db_ftell(flptr);
        if (handler(flptr) == -1) {
            DbgPrint("SaveSlot: Error - handler[%d] failed to save\n", i);
        ...
        DbgPrint("SaveSlot: handler[%d] wrote %ld bytes\n", i, db_ftell(flptr) - pos);
    }

    DbgPrint("SaveSlot: Total save size: %ld bytes\n", db_ftell(flptr));
    db_fclose(flptr);
    ...
    if (message_search(&lsgame_msgfl, &lsgmesg)) {
        ...
        DbgPrint("SaveSlot: Error - could not find message 140\n");
    }

    gsound_background_unpause();
    DbgPrint("SaveSlot: Complete\n");
...
// 0x471070
// Dirty for Xbox Saves
static int get_input_str2(int win, int doneKeyCode, int cancelKeyCode, char* description, int maxLength, int x, int y, int textColor, int backgroundColor, int flags)
{
    snprintf(description, maxLength, "Save Slot %d", slot_cursor + 1);
    return 0; // Simulate successful input
}
