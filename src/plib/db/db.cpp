...
{
    DbgPrint("db_init: entered\n");
    DbgPrint("db_init: datafile = %s\n", datafile);
    DbgPrint("db_init: datafile_path = %s\n", datafile_path);
    DbgPrint("db_init: patches_path = %s\n", patches_path);

    DB_DATABASE* database;

    DbgPrint("db_init: calling db_create_database\n");
    if (db_create_database(&database) != 0) {
        DbgPrint("db_init: db_create_database failed\n");
        return INVALID_DATABASE_HANDLE;
    }

    DbgPrint("db_init: calling db_init_database\n");
    if (db_init_database(database, datafile, datafile_path) != 0) {
        DbgPrint("db_init: db_init_database failed\n");
        db_close(database);
        return INVALID_DATABASE_HANDLE;
    }

    DbgPrint("db_init: calling db_init_patches\n");
    if (db_init_patches(database, patches_path) != 0) {
        DbgPrint("db_init: db_init_patches failed\n");
        db_close(database);
        return INVALID_DATABASE_HANDLE;
    }

    if (current_database == NULL) {
        DbgPrint("db_init: setting current_database\n");
        current_database = database;
    }

    if (hash_is_on) {
        DbgPrint("db_init: hash table is enabled, calling db_init_hash_table\n");
        if (db_init_hash_table(database) != 0) {
            DbgPrint("db_init: db_init_hash_table failed, clearing hash_table\n");
    ...
    DbgPrint("db_init: exiting successfully\n");
    return database;
}
...
static int db_init_database(DB_DATABASE* database, const char* datafile, const char* datafile_path)
{
    ...
    if (datafile == NULL) {
        DbgPrint("db_init_database: datafile is NULL, returning success\n");
        return 0;
    }

    DbgPrint("db_init_database: duplicating datafile string\n");
    database->datafile = internal_strdup(datafile);
    if (database->datafile == NULL) {
        DbgPrint("db_init_database: internal_strdup failed\n");
        return -1;
    }

    DbgPrint("db_init_database: opening datafile stream: %s\n", database->datafile);
    database->stream = compat_fopen(database->datafile, "rb");
    if (database->stream == NULL) {
        DbgPrint("db_init_database: compat_fopen failed\n");
        internal_free(database->datafile);
        database->datafile = NULL;
        return -1;
    }

    DbgPrint("db_init_database: initializing root assoc\n");
    if (assoc_init(&(database->root), 0, sizeof(*database->entries), NULL) != 0) {
        DbgPrint("db_init_database: assoc_init failed\n");
    ...
    DbgPrint("db_init_database: loading root assoc\n");
    if (assoc_load(database->stream, &(database->root), 0) != 0) {
        DbgPrint("db_init_database: assoc_load failed on root\n");
        fclose(database->stream);
        internal_free(database->datafile);
        database->datafile = NULL;
        return -1;
    ...
    DbgPrint("db_init_database: allocating entries array, size = %d\n", database->root.size);
    database->entries = (assoc_array*)internal_malloc(sizeof(*database->entries) * database->root.size);
    if (database->entries == NULL) {
        DbgPrint("db_init_database: internal_malloc for entries failed\n");
    ...
    DbgPrint("db_init_database: setting assoc_func_list\n");
    ...
    for (index = 0; index < database->root.size; index++) {
        DbgPrint("db_init_database: assoc_init entry %d\n", index);
        if (assoc_init(&(database->entries[index]), 0, sizeof(dir_entry), &funcs) != 0) {
            DbgPrint("db_init_database: assoc_init failed on index %d\n", index);
            break;
        }

        if (assoc_load(database->stream, &(database->entries[index]), 0) != 0) {
            DbgPrint("db_init_database: assoc_load failed on index %d\n", index);
            break;
        }
    }

    if (index < database->root.size) {
        DbgPrint("db_init_database: error during entry load loop at index %d\n", index);
        while (--index >= 0) {
    ...
    v2 = strlen(v1);
    DbgPrint("db_init_database: datafile_path = %s (len = %d)\n", v1, (int)v2);

    database->datafile_path = (char*)internal_malloc(v2 + 2);
    if (database->datafile_path == NULL) {
        DbgPrint("db_init_database: malloc for datafile_path failed\n");
        internal_free(database->entries);
        assoc_free(&(database->root));
        fclose(database->stream);
        internal_free(database->datafile);
    ...
    DbgPrint("db_init_database: success!\n");
    return 0;
}
