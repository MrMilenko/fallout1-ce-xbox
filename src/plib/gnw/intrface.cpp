...
void GNW_intr_init()
{
    // DbgPrint("GNW_intr_init(): starting\n");
    ...
    // DbgPrint("GNW_intr_init(): scr_center_x = %d\n", scr_center_x);
    ...
    // Original: tm_h = 2 * tm_text_y + text_height();
    tm_h = 2 * tm_text_y + 10;  // Faked value to avoid null function pointer crash
    // DbgPrint("GNW_intr_init(): tm_h = %d (text_height = [stubbed 10])\n", tm_h);

    int v1 = scr_size.lry >> 3;
    int v2 = scr_size.lry >> 2;

    for (int i = 0; i < 5; i++) {
        // DbgPrint("GNW_intr_init(): tm_location[%d].y = %d\n", i, v1 * i + v2);
        tm_location[i].y = v1 * i + v2;
        tm_location[i].taken = 0;
    }

    // DbgPrint("GNW_intr_init(): complete\n");
}
