...
{
    // DbgPrint("win_init: entered\n");

#ifdef _WIN32
    // DbgPrint("win_init: Closing GNW95_mutex\n");
    CloseHandle(GNW95_mutex);
    GNW95_mutex = INVALID_HANDLE_VALUE;
#endif

    if (GNW95_already_running) {
        // DbgPrint("win_init: GNW95_already_running is true\n");
        return WINDOW_MANAGER_ERR_ALREADY_RUNNING;
    }

#ifdef _WIN32
    if (GNW95_title_mutex == INVALID_HANDLE_VALUE) {
        // DbgPrint("win_init: GNW95_title_mutex is INVALID\n");
        return WINDOW_MANAGER_ERR_TITLE_NOT_SET;
    }
#endif

    if (GNW_win_init_flag) {
        // DbgPrint("win_init: GNW_win_init_flag already set\n");
        return WINDOW_MANAGER_ERR_WINDOW_SYSTEM_ALREADY_INITIALIZED;
    }

    // DbgPrint("win_init: resetting window_index[]\n");
    for (int index = 0; index < MAX_WINDOW_COUNT; index++) {
        window_index[index] = -1;
    }

    if (db_total() == 0) {
        // DbgPrint("win_init: db_total == 0, calling db_init()\n");
        if (db_init(NULL, NULL, "", 1) == INVALID_DATABASE_HANDLE) {
            // DbgPrint("win_init: db_init failed\n");
            return WINDOW_MANAGER_ERR_INITIALIZING_DEFAULT_DATABASE;
        }
    }

    // DbgPrint("win_init: calling GNW_text_init()\n");
    if (GNW_text_init() == -1) {
        // DbgPrint("win_init: GNW_text_init failed\n");
        return WINDOW_MANAGER_ERR_INITIALIZING_TEXT_FONTS;
    }

    // DbgPrint("win_init: calling svga_init()\n");
    if (!svga_init(video_options)) {
        // DbgPrint("win_init: svga_init failed, calling svga_exit()\n");
        svga_exit();
        return WINDOW_MANAGER_ERR_INITIALIZING_VIDEO_MODE;
    }

    if ((flags & 1) != 0) {
        int w = scr_size.lrx - scr_size.ulx + 1;
        int h = scr_size.lry - scr_size.uly + 1;
        // DbgPrint("win_init: allocating screen_buffer size = %dx%d = %d\n", w, h, w * h);
        screen_buffer = (unsigned char*)mem_malloc(w * h);
        if (screen_buffer == NULL) {
            // DbgPrint("win_init: screen_buffer allocation failed\n");
            svga_exit();
    ...
    // DbgPrint("win_init: calling colorInitIO + colorRegisterAlloc\n");
    colorInitIO(colorOpen, colorRead, colorClose);
    colorRegisterAlloc(mem_malloc, mem_realloc, mem_free);

    // DbgPrint("win_init: calling initColors\n");
    if (!initColors()) {
        // DbgPrint("win_init: initColors failed, allocating fallback palette\n");
        unsigned char* palette = (unsigned char*)mem_malloc(768);
        if (palette == NULL) {
            // DbgPrint("win_init: fallback palette allocation failed\n");
            svga_exit();
            if (screen_buffer != NULL) {
        ...
        // DbgPrint("win_init: filling fallback palette with 0s\n");
        buf_fill(palette, 768, 1, 768, 0);
        // TODO: missing _colorBuildColorTable()

        mem_free(palette);
    }

    // DbgPrint("win_init: calling GNW_debug_init\n");
    GNW_debug_init();

    // DbgPrint("win_init: calling GNW_input_init\n");
    if (GNW_input_init(flags) == -1) {
        // DbgPrint("win_init: GNW_input_init failed\n");
        return WINDOW_MANAGER_ERR_INITIALIZING_INPUT;
    }

    // DbgPrint("win_init: calling GNW_intr_init\n");
    GNW_intr_init();

    // DbgPrint("win_init: allocating window[0]\n");
    Window* w = window[0] = (Window*)mem_malloc(sizeof(*w));
    if (w == NULL) {
        // DbgPrint("win_init: window[0] allocation failed\n");
        svga_exit();
        if (screen_buffer != NULL) {
            mem_free(screen_buffer);
        }
    ...
    // DbgPrint("win_init: filling window[0] structure\n");
    w->id = 0;
    w->flags = 0;
    ...
    // DbgPrint("win_init: calling win_no_texture and registering atexit\n");
    win_no_texture();
    atexit(win_exit);

    // DbgPrint("win_init: success!\n");
...
{
    // DbgPrint("win_show: called with win=%d\n", win);

    if (!GNW_win_init_flag) {
        // DbgPrint("win_show: GNW_win_init_flag is false, aborting\n");
        return;
    }

    Window* w = GNW_find(win);
    if (w == NULL) {
        // DbgPrint("win_show: GNW_find(%d) returned NULL, aborting\n", win);
        return;
    }

    int v3 = window_index[w->id];
    // DbgPrint("win_show: window id=%d, index=%d\n", w->id, v3);

    if (w->flags & WINDOW_HIDDEN) {
        w->flags &= ~WINDOW_HIDDEN;
        // DbgPrint("win_show: window was hidden, un-hiding\n");

        if (v3 == num_windows - 1) {
            // DbgPrint("win_show: window is topmost, refreshing\n");
    ...
    if (v3 < v5 && !(w->flags & WINDOW_DONT_MOVE_TOP)) {
        ...
        // DbgPrint("win_show: moved window to top, refreshing\n");
        GNW_win_refresh(w, &(w->rect), NULL);
    }

    // DbgPrint("win_show: done\n");
}
...
{
    if (!GNW_win_init_flag) {
        // DbgPrint("win_hide: window system not initialized\n");
        return;
    }

    Window* w = GNW_find(win);
    if (w == NULL) {
        // DbgPrint("win_hide: window not found (id=%d)\n", win);
        return;
    }

// DbgPrint("win_hide: window found (id=%d), flags=0x%X, rect={ulx=%d uly=%d lrx=%d lry=%d}\n",
 //   win, w->flags, w->rect.ulx, w->rect.uly, w->rect.lrx, w->rect.lry);


    if ((w->flags & WINDOW_HIDDEN) == 0) {
        w->flags |= WINDOW_HIDDEN;
        // DbgPrint("win_hide: window not hidden, calling refresh_all\n");
        refresh_all(&(w->rect), NULL);
    } else {
        // DbgPrint("win_hide: window already hidden\n");
    }
}
