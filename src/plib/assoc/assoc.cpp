...
int assoc_load(FILE* fp, assoc_array* a, int flags)
{
    ...
    if (a->list != NULL) {
        for (int index = 0; index < a->size; index++) {
            assoc_pair* entry = &(a->list[index]);
            if (entry->name != NULL) internal_free(entry->name);
            if (entry->data != NULL) internal_free(entry->data);
        }
        internal_free(a->list);
        a->list = NULL;
    }

    // Load assoc_array metadata
    if (assoc_read_assoc_array(fp, a) != 0) {
        // // DbgPrint("assoc_load: assoc_read_assoc_array failed\n");
        return -1;
    }

    // // DbgPrint("assoc_load: assoc_array parsed, size=%d max=%d datasize=%d\n", a->size, a->max, a->datasize);

    if (a->max <= 0 || a->size <= 0) {
        // // DbgPrint("assoc_load: nothing to load (max or size <= 0)\n");
        return 0;
    }

    a->list = (assoc_pair*)internal_malloc(sizeof(*a->list) * a->max);
    if (!a->list) {
        // // DbgPrint("assoc_load: failed to alloc assoc_pair list\n");
        return -1;
    }
    ...
    for (int index = 0; index < a->size; index++) {
        ...
        uint8_t keyLengthByte;
        if (fread(&keyLengthByte, 1, 1, fp) != 1) {
            // // DbgPrint("assoc_load: failed to read keyLength at index %d\n", index);
            return -1;
        }
        int keyLength = (int)keyLengthByte;


        if (keyLength <= 0 || keyLength > 255) {
            long pos = ftell(fp);
            // // DbgPrint("assoc_load: invalid keyLength=%d at index %d (ftell=%ld)\n", keyLength, index, pos);


            unsigned char peek[8];
            fread(peek, 1, 8, fp);
            // // DbgPrint("assoc_load: peek bytes = %02X %02X %02X %02X %02X %02X %02X %02X\n",
            //    peek[0], peek[1], peek[2], peek[3], peek[4], peek[5], peek[6], peek[7]);
            fseek(fp, pos + 1, SEEK_SET); // rewind back after peek
            return -1;
        }

        entry->name = (char*)internal_malloc(keyLength + 1);
        if (!entry->name) {
            // // DbgPrint("assoc_load: failed to alloc name at index %d\n", index);
            return -1;
        }

        if (fread(entry->name, 1, keyLength, fp) != keyLength) {
            // // DbgPrint("assoc_load: failed to read name at index %d\n", index);
            return -1;
        }
        entry->name[keyLength] = '\0'; 

        //// // DbgPrint("assoc_load: index %d name='%s'\n", index, entry->name);

        if (a->datasize > 0) {
            entry->data = internal_malloc(a->datasize);
            if (!entry->data) {
                // // DbgPrint("assoc_load: data malloc failed at index %d\n", index);
                return -1;
            }

            if (a->load_save_funcs.loadFunc != NULL) {
                if (a->load_save_funcs.loadFunc(fp, entry->data, a->datasize, flags) != 0) {
                    // // DbgPrint("assoc_load: loadFunc failed at index %d\n", index);
                    return -1;
                }
            } else {
                if (fread(entry->data, a->datasize, 1, fp) != 1) {
                    // // DbgPrint("assoc_load: fread failed at index %d\n", index);
                    return -1;
                }
            }
        }
    ...
    // // DbgPrint("assoc_load: completed successfully\n");
    return 0;
}
