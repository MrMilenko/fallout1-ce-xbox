...
{
    // DbgPrint("fadeSystemPalette: start, steps=%d\n", steps);

    // Clamp steps to a sane range
    if (steps <= 0 || steps > 128) {
        steps = 32; // reasonable default
        // DbgPrint("fadeSystemPalette: clamped steps to %d\n", steps);
    }

    const int totalFadeDurationMs = 500;
    const int delayPerStep = std::max(1, totalFadeDurationMs / steps);

    for (int step = 0; step < steps; step++) {
        unsigned char palette[768];

        for (int i = 0; i < 768; i++) {
            int diff = newPalette[i] - oldPalette[i];
            palette[i] = oldPalette[i] + diff * step / steps;
        }
        ...
        SDL_Delay(delayPerStep);
    }

    // DbgPrint("fadeSystemPalette: applying final palette\n");
    setSystemPalette(newPalette);
    renderPresent();
    SDL_Delay(10);

    // DbgPrint("fadeSystemPalette: done\n");
}
