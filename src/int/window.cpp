...
void initWindow(VideoOptions* video_options, int flags)
{
    ...
    // DbgPrint("initWindow: entered\n");
    // DbgPrint("initWindow: calling interpretRegisterProgramDeleteCallback\n");

    interpretRegisterProgramDeleteCallback(windowRemoveProgramReferences);

    // DbgPrint("initWindow: setting default text colors and flags\n");
    currentTextColorR = 0;
    currentTextColorG = 0;
    currentTextColorB = 0;
    ...
    yres = 640;
    xres = 480;
    // DbgPrint("initWindow: resolution set to %dx%d\n", xres, yres);

    for (int i = 0; i < MANAGED_WINDOW_COUNT; i++) {
        windows[i].window = -1;
    }
    // DbgPrint("initWindow: cleared window handles\n");

    // DbgPrint("initWindow: calling win_init\n");
    rc = win_init(video_options, flags);
    // DbgPrint("initWindow: win_init returned %d\n", rc);
    ...
    if (rc != WINDOW_MANAGER_OK) {
        ...
        case WINDOW_MANAGER_ERR_8:
            snprintf(err, sizeof(err), "Unknown WINDOW_MANAGER_ERR_8\n");
            break;
        ...
        // DbgPrint("initWindow: win_init error: %s", err);
        GNWSystemError(err);
        exit(1);
    }

    // DbgPrint("initWindow: win_init succeeded\n");

    currentFont = 100;
    // DbgPrint("initWindow: calling text_font(100)\n");
    text_font(100);

    // DbgPrint("initWindow: calling initMousemgr\n");
    initMousemgr();

    // DbgPrint("initWindow: setting mouse name mangler\n");
    mousemgrSetNameMangler(interpretMangleName);

    // DbgPrint("initWindow: populating alphaBlendTable\n");
    for (i = 0; i < 64; i++) {
        for (j = 0; j < 256; j++) {
            alphaBlendTable[(i << 8) + j] = ((i * j) >> 9);
        }
    }
    // DbgPrint("initWindow: alphaBlendTable populated\n");

    // DbgPrint("initWindow: completed\n");
}
